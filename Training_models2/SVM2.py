...
df = pd.read_csv("../dataset/newsyllableCV.csv", nrows=200000)
...
svm_model = SVC(kernel='poly', C=1.0) # linear, rbf, poly
svm_model.fit(X_train_words, np.asarray(y_train_syllables.argmax(axis=1)))

# Predict on the test set
y_pred = svm_model.predict(X_test_words)

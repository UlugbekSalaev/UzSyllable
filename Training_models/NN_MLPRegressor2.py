...
data = pd.read_csv('../dataset/dataset_new_latin.csv')
...
# Cross-validation
cv_scores = cross_val_score(mlp_model, X_train, y_train, cv=10)
...
while True:
    ...
    new_word_vowels = sum([1 for char in new_word if char in ['a', 'i', 'e', 'o', 'u', 'ō']])
    predicted_syllables = mlp_model.predict([[new_word_vowels]])[0]
